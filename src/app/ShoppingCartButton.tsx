...
import { useCart, useRemoveCartItem, useUpdatedCartItemQuantity } from "@/hooks/cart";
  ...
  return (
    <>
      <div className="relative">
        <Button
          onClick={() => setSheetOpen(!sheet)}
          variant="ghost"
          size="icon"
        >
          <ShoppingCartIcon />
          <span
            className={`absolute right-0 top-0 flex size-5 items-center justify-center rounded-full bg-primary text-xs text-primary-foreground`}
          >
            {totalQuantity < 10 ? totalQuantity : "9+"}
          </span>
        </Button>
      </div>
      <Sheet open={sheet} onOpenChange={setSheetOpen}>
          ...
          <div className="flex grow flex-col space-y-5 overflow-y-auto pt-1">
            ...
            {cartQuery.error && (
              <p className="text-destructive">
                {cartQuery.error.message || "Erro ao carregar o carrinho"}
              </p>
            )}

            {!cartQuery.isPending && !cartQuery.data?.lineItems?.length && (
              <div className="grown flex items-center justify-center text-center">
...
function ShoppingCartItem({ item, onProductLinkClicked }: ShoppingCartItemProps) {
  const updateQuantityMutation = useUpdatedCartItemQuantity();
    ...
    <li className="flex items-center gap-3">
      ...
      <div className="space-y-1.5 text-sm">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {quantityLimitReached && <span>Limite atingindo</span>}
        </div>
      </div>
    </li>
  );
